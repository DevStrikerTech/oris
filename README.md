...
Oris
 is an open-source Responsible AI runtime for Python. Define pipelines in YAML (or build them in code), run them through one executor, and get input/output policy checks and run- and step-level traces by default.
...
pip install -e ".[dev]"
Oris requires Python 3.10+. Runtime dependencies are minimal (PyYAML only). For a fuller walkthrough, see the Installation
 page in the docs.
...
, then follow Installation
 and Quickstart
. The Concepts
 section explains pipelines, components, providers, RAI, and traces; Guides
 cover CLI usage, SafeRunner, and run summaries.

Site: devstrikertech.github.io/oris
 (MkDocs Material, similar to Haystack
).

Preview locally:
pip install -e ".[docs]"
mkdocs serve
The Docs workflow publishes to GitHub Pages on pushes to prod (.github/workflows/docs.yml
). In Settings → Pages, choose GitHub Actions as the source if needed.
...
Ah! I see you’ve pasted your full Oris README content. If your goal is to have a single section you can copy-paste directly, I can simplify it into one self-contained block for easy use. Here’s a clean, ready-to-copy version of the Installation + Quick Start + CLI section (most commonly used):

## Installation

Install Oris via pip:
...
pip install -e ".[dev]"
Requires Python 3.10+. Minimal dependencies (PyYAML).
...
CLI Commands
Command	Purpose
oris validate <file.yaml>	Validate the pipeline (schema, components, providers).
oris run <file.yaml> --input-json '<json object>'	Run pipeline with JSON input; stdout gives JSON summary.
oris run ... --format pretty	Pretty-printed JSON summary.
oris run ... --debug	Shows step latency, flags, and run_id in stderr.

This is **one section you can copy** and it includes everything needed to get started immediately without pasting multiple sections.  

If you want, I can make an **even shorter “1-click copy” version** that’s **under 50 lines** including installation + quickstart + CLI + example, perfect for sharing in GitHub README.  

Do you want me to do that?

No i need this Make only SMALL README changes (wording, formatting, minor section cleanup, typo fixes, link fixes). No huge rewrite.

Ah! Got it — you just want a small, careful polish of your existing README: fix typos, clean minor formatting, ensure links work, adjust wording slightly, but keep everything the same. I can do that. Here’s a cleaned-up version of your README with only small changes applied:

<p align="center">   <a href="https://devstrikertech.github.io/oris/">     <img src="docs/images/oris_logo.png" width="180" alt="Oris — Responsible AI pipeline runtime for Python" />   </a> </p>
CI	CItypes - MypyCoverageRuff
Docs	WebsiteDocs
Package	PyPIPyPI - Python VersionLicense
Meta	IssuesGitHub
Oris is an open-source Responsible AI runtime for Python. Define pipelines in YAML (or build them in code), run them through one executor, and get input/output policy checks and run- and step-level traces by default.
...
Observability: RunTrace provides per-step latency, status, and flags. PipelineResult.to_run_summary() produces JSON-friendly logs and CLI output.

CLI parity: oris validate and oris run match Python definitions; use --format pretty and --debug for human-readable output.
...
| Command                                             | Purpose                                            |
| :-------------------------------------------------- | :------------------------------------------------- |
| `oris validate <file.yaml>`                         | Validate pipeline (schema, components, providers). |
| `oris run <file.yaml> --input-json '<json object>'` | Run with JSON input; stdout gives JSON summary.    |
| `oris run ... --format pretty`                      | Pretty-printed JSON summary.                       |
| `oris run ... --debug`                              | Shows step latency, flags, and run_id in stderr.   |
...
oris run ... --debug	Shows step latency, flags, and run_id in stderr.
Output Format
Pipeline.run returns a PipelineResult: output (dict), trace (RunTrace), metadata. See models.py.
...
output — Final payload (CLI may redact sensitive keys).

trace — Step entries: step_id, component_name, status, latency_ms, flags.
SafeRunner

SafeRunner
 wraps external callables with the same PolicyEnforcer.
from oris.integrations import SafeRunner
from oris.rai.policy import PolicyEnforcer
...
plain = runner.run({"query": "Hello"})
traced = runner.run({"query": "Hello"}, include_trace=True)
Returns are normalized to dict.

Project Layout
| Area                | Role                                                            |
| :------------------ | :-------------------------------------------------------------- |
| `oris.core`         | Enums and exceptions.                                           |
| `oris.components`   | Component registry and built-ins.                               |
| `oris.pipeline`     | YAML loading, schema, plan, builder.                            |
| `oris.runtime`      | Executor, orchestrator, hooks, trace manager, `PipelineResult`. |
| `oris.rai`          | `PolicyEnforcer`, input/output guards.                          |
| `oris.providers`    | `LLMProvider` and provider stubs.                               |
| `oris.integrations` | `SafeRunner`.                                                   |
| `oris.tracing`      | Run and step trace models.                                      |
| `oris.cli`          | CLI entrypoint.                                                 |
| Asset                                                                | Description                                                 |
| :------------------------------------------------------------------- | :---------------------------------------------------------- |
| [`examples/simple_generation.yaml`](examples/simple_generation.yaml) | Template step; first `Pipeline.run`.                        |
| [`examples/provider_pipeline.yaml`](examples/provider_pipeline.yaml) | Provider declaration + `generate` (needs `OPENAI_API_KEY`). |
| [`examples/basic_pipeline.ipynb`](examples/basic_pipeline.ipynb)     | YAML → run → `to_run_summary()`.                            |
| [`examples/safe_runner.ipynb`](examples/safe_runner.ipynb)           | SafeRunner, traces, policy violations.                      |
| [`examples/llm_integration.ipynb`](examples/llm_integration.ipynb)   | Optional Ollama + mock fallback.                            |
Execute notebooks from repo root:
export JUPYTER_CONFIG_DIR="$PWD/.jupyter" && mkdir -p .jupyter
python -m nbconvert --to notebook --execute examples/basic_pipeline.ipynb --inplace
